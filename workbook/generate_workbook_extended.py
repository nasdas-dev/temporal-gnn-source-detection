...
\section{Temporal Percolation and Thresholding ($D_{\Delta t}$)}

A major application highlighted in the paper is \textbf{Temporal Network Percolation}. By discarding all edges in $D$ where the weight (time gap) is greater than $\Delta t$, we create a thresholded event graph $D_{\Delta t}$.

\begin{analogybox}{The Relay Race Analogy}
Imagine a relay race where runners pass a baton. However, if a runner waits more than $\Delta t$ seconds after receiving the baton, they drop it and are disqualified. The thresholded graph $D_{\Delta t}$ perfectly models this: it only contains the pathways where the baton was successfully handed off in time. 
In epidemiology, this perfectly models diseases with a finite infectious period. If you don't interact with someone within $\Delta t$ days of getting sick, you recover and the transmission chain dies.
\end{analogybox}

By analyzing the weakly connected components of $D_{\Delta t}$, one can find the \emph{critical point} of the network---the exact value of $\Delta t$ at which a giant temporal component emerges, allowing an epidemic to sweep the entire population.
...
\section{Encoding the Input: Time as a Dense Vector}

Rather than building a complex DAG or De Bruijn graph, the Backtracking Network (BN) maintains the standard static graph topology but injects time via dense feature vectors on the edges.

\begin{mathbox}{Encoding Available Information}
1. \textbf{Final State (Nodes):} The state of an individual $i$ is one-hot encoded: $C_i \in \{[1,0,0], [0,1,0], [0,0,1]\}$. These are initially projected to a $D$-dimensional hidden space:
\[
h_i^{(0)} = p^v(C_i) = \text{ReLU}(\mathbf{W}^v C_i)
\]

2. \textbf{Temporal Activation Patterns (Edges):} The total time span $T$ is divided into discrete slices. For each edge $e = (i,j)$, we create a binary vector $X_e \in \{0, 1\}^T$ where $X_e[t] = 1$ if the edge was active at time $t$, and $0$ otherwise. These are projected similarly:
\[
g_{(i,j)}^{(0)} = p^e(X_{(i,j)}) = \text{ReLU}(\mathbf{W}^e X_{(i,j)})
\]
\end{mathbox}

\section{The Interdependent Message Passing Logic}

The key innovation of the BN architecture is the \textbf{interdependent update}. In standard GNNs (like GCN or GraphSAGE), edges only act as structural routes; they do not hold or update data. In BN, edges have their own hidden state that evolves layer by layer based on the state of the node they originate from.

\begin{analogybox}{The Telephone Game with Smart Wires}
Imagine a telephone game. Usually, Alice whispers to Bob (standard GNN). In the Backtracking Network, the "wire" connecting Alice to Bob is a smart wire. Before passing the message to Bob, the wire looks at its own history ("Was I even plugged in during the likely infection window?") and looks at Alice's current state ("Is Alice actually sick?"). It updates its own internal state to decide how important this message is, and \emph{then} passes it to Bob.
\end{analogybox}

\begin{mathbox}{Layer Updates (Eq. 3 and 4 in Ru et al.)}
...
\]
The edge embedding $g_{(i,j)}$ is updated by concatenating its previous state with the current state of the \emph{source node} $h_i^{(l)}$. Here, $f^e$ is an MLP (Fully Connected Layer + ReLU).
...
\section{Expert Knowledge Injection: The Susceptible Mask}
...
\begin{mathbox}{Eq. 6: Forcing Probabilities (Expert Knowledge)}
Let $h_i \in \mathbb{R}^1$ be the scalar output score for node $i$ from the final layer. We apply expert knowledge:
\[
h_s \gets h_s - \infty \quad \text{for all susceptible nodes } s
\]
When passed through the final softmax function:
\[
P(\text{source}=i) = \frac{\exp(h_i)}{\sum_{j} \exp(h_j)}
\]
For a susceptible node $s$, $\exp(-\infty) = 0$, strictly enforcing that $P(\text{source}=s) = 0$. This prevents the network from wasting probability mass on impossible candidates.
\end{mathbox}

\section{PyTorch Implementation: The Scatter Add Trick}

In our codebase, how do we efficiently compute the node update $\sum_{j \in \mathcal{N}(i)} g_{(j,i)}^{l+1}$? We use \texttt{torch.scatter\_add\_}. This is a highly optimized C++/CUDA operation under the hood of PyTorch.
...
def forward(self, h, g, edge_index):
    ...
    g_new = self.f_e(torch.cat([g, h_src], dim=-1)) # [B, E, D] -> [B, E, 2D] -> f_e -> [B, E, D]
...
\section{Robustness to Missing Information}
Ru et al. perform experiments where they randomly delete edges from the contact network, or hide the SIR state of certain nodes (Partial Observation). They prove that the Backtracking Network is incredibly robust to partial observations (even if 80\% of nodes are unknown, top-5 accuracy remains $\sim 70\%$). However, the model breaks down much faster when \emph{contact edges} are missing, because the temporal activation patterns $X_e$ become corrupted, destroying the causal routing.
...
If we collapse a temporal network into a static graph, message passing simulates a random walk. A message arrives at node $B$, and node $B$ passes it to all neighbors $A, C, D$. This is a \textbf{First-Order Markov process}: the next step depends \emph{only} on the current node ($B$), with no memory of where the message came from.

However, in a dynamic graph, if a message arrived from $A \to B$ at time $t=1$, it can only proceed to $C$ if the $B \to C$ contact happens at $t > 1$. The valid next step depends on the \emph{history} of the path. This requires a \textbf{Higher-Order (Non-Markovian)} model.

\section{The De Bruijn Transformation}

To force a memoryless algorithm (like a GNN) to respect history, we change the graph itself. We move to a higher-dimensional topological space.
...
\end{itemize}
Message passing on $G^{(k)}$ is now strictly confined to valid chronological paths!

\section{Model Selection: Choosing $k$}

How do we know if $k=2$ is enough? Or if we need $k=3$? The authors use statistical model selection based on Likelihood Ratio Tests. They count the frequencies of causal paths of length $k$ in the data. If a 1st-order graph (static graph) can statistically explain the frequencies of length-2 paths, then $k=1$ is sufficient. If not, $p < 0.05$, we reject the null hypothesis and move to $k=2$. For almost all empirical human contact networks, $k=2$ is statistically required.

\section{The Sentinel Node Trick and Bipartite Mapping}
...
The paper proposes a complex "Bipartite projection layer" (Equation 2) to map event embeddings back to original nodes:
\[
\tilde{h}_v^b = \sigma \left( \mathbf{W}^b \mathcal{F}\left( \{ \tilde{h}_u^{k,l} + \tilde{h}_v^{1,g} : \text{for } u \in V^{(k)} \text{ with } (u,v) \in E^b \} \right) \right)
\]
In our codebase, we achieve this more elegantly using \textbf{Sentinel Nodes}.
...
To definitively prove that GNNs are learning something profound, we must evaluate them against established mathematical baselines.

\section{The Soft Margin Estimator (SME)}

Proposed by Antulov-Fantulin et al. (2015), SME is our most rigorous probabilistic baseline. It embraces the stochasticity of the system using Monte Carlo Kernel Density Estimation.

For every candidate node $q$, we run $n$ simulations to generate simulated final snapshots $r_{q,i}$. We compare these simulated snapshots to the real observed snapshot $r^*$.
...
\]
Treating this similarity as a random variable, the empirical probability density function is smoothed using a Gaussian kernel $w_a(x) = \exp(-(x-1)^2/a^2)$. The estimated likelihood for source $q$ is the expected value of the kernel:
\[
\hat{P}(r^* \mid q) = \frac{1}{n} \sum_{i=1}^n \exp\left(-\frac{(\varphi(r^*, r_{q,i}) - 1)^2}{a^2}\right)
\]
As sample size $n \to \infty$ and $a \to 0$, this mathematically converges to the true Monte Carlo likelihood estimator.
\end{mathbox}

\section{Monte Carlo Mean Field (MCMF)}

MCMF (Sterchi et al.) is a faster, highly competitive baseline. Instead of comparing whole snapshots (which requires a massive $n$ to find an exact match), it calculates marginal probabilities for each node independently.
...
\]
Because multiplying many small probabilities causes extreme numerical underflow, we compute the log-likelihood:
\[
...
\begin{itemize}
    \item \textbf{Top-K Accuracy:} $\mathbb{I}[R \le K]$. The percentage of test instances where the true source was in the top K guesses. Top-1 is exact accuracy. Top-5 gives credit for being very close.
    \item \textbf{Mean Reciprocal Rank (Rank Score):} $\frac{1}{N_{runs}} \sum \frac{1}{R_i}$. This metric heavily penalizes severe misses. Ranking the source 2nd gives a score of 0.5. Ranking it 100th gives a score of 0.01.
    \item \textbf{Error Distance:} The shortest path distance in the static network between the model's top guess (MAP estimate) and the true source. If the algorithm is wrong, is it at least geographically close?
    \item \textbf{90\% Credible Set Size (CSS):} Sort nodes by probability and sum them up until $0.90$ is reached. How many nodes did it take? A smaller CSS indicates higher confidence and sharper predictive distributions.
\end{itemize}
...
print("Created expanded workbook.tex")
