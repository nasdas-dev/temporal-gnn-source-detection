...
typedef struct GLOBALS {
	...
	// TIME
    float t, T;
    unsigned int samp_T; // this is to define whether T should be sampled or not
	// FOR RNG
	uint64_t state;
	float rexp[0x10000], rT[0x10000];
} GLOBALS;

typedef struct NODE {
	unsigned int deg, *nb, *weights; // degree, network neighbors, and their weights
	unsigned int heap;
    unsigned int infby; // to store who a node way infected by (for R0 comp.)
	float time;
} NODE;
