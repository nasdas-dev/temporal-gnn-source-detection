...
class DBGNN(torch.nn.Module):
    ...
    def forward(
        ...
        if n_db > 0:
            ...
            # Empty De Bruijn graph (no causal walk completions; e.g. a fully
            # time-collapsed bin width). Fall back to the first-order static
            # branch so the model degrades to a genuine static GCN instead of a
            # bias-only, ~uniform predictor. The graph builder drops the
            # higher-order branch (n_db=0) whenever there are no causal edges,
            # for every order, so this fallback is reached identically by k=2 and
            # k=3 at collapse — the collapse endpoint is order-invariant.
            bipartite_agg = h_fo
