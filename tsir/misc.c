...
unsigned int exptime () {
	...
	if (r == 4294967295u) return 0; // otherwise r+1 would overflow
	if (g.lambda == 0.0) return END; // if lambda is zero, return infinity

    // Note that if U ~ Unif(0, 1) then -1/lambda * log(U) ~ Exp(lambda)
    // ceil is important, otherwise a node could recover at the same time step as it got infected
    return ceil(-(1.0 / g.lambda) * log((r + 1) / 4294967296.0));
}
