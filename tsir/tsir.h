...
typedef struct GLOBALS {
	// INPUT PARAMETERS
	double lambda; // recovery rate that is computed from the recovery probability mu
	unsigned short rnd2inx[0x10000]; // mapping 16-bit random number to index for infection probabilities
	// NETWORK SPECS
	unsigned int n, dur;
	// SIMULATION CONFIGURATION
	unsigned int start_t, stop_t;  // start and stop simulation at this time
	// OTHER GLOBALS
	unsigned int nheap, *heap;
	// FOR RND
	uint64_t state;
	// OUTBREAK STATS
	unsigned int ns, *s, r0;
} GLOBALS;
...
typedef struct NODE {
	...
	unsigned int infby; // index of the node that infected this node, or NONE if not infected
} NODE;
