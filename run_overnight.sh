...
# Parameter sets: "beta mu tag"
# end_t is NOT overridden — each network uses its own t_max from tsir.yml
PARAM_SETS=(
    "0.24 0.01 b024"
    "0.16 0.01 b016"
    "0.11 0.01 b011"
...
for DATASET in $NETWORKS; do
    for PARAMS in "${PARAM_SETS[@]}"; do
        read -r BETA MU TAG <<< "$PARAMS"
        ARTIFACT="${DATASET}_${TAG}"
        CFG_TMP="exp/${DATASET}/tsir_${TAG}.yml"

        echo ""
        echo "============================================================"
        echo "  Dataset: $DATASET  |  beta=$BETA  mu=$MU  (end_t from tsir.yml)"
        echo "============================================================"

        # --- Step 0: Write a temporary tsir config with patched parameters ---
        # end_t is kept from the original tsir.yml (network-specific full duration)
