...
Protocol (mirrors Sterchi et al., "GNNs for Source Detection: A Review and
Benchmark Study", arXiv:2512.20657, Section 5.3 / 6.2.3)
--------------------------------------------------------------------------
...
* **Multi-seed final evaluation** — the ``tuner`` preset trains and evaluates
  ...
  average, matching the paper's three-seed protocol.
...
* **Network-scope tuning at R0 ~= 2** — like the paper, hyperparameters are tuned
  once per network/model at a single reference epidemic regime (``r0_20``) and
  reused across the full R0 sweep, keeping the search faithful and tractable.

Model selection uses the downstream task metric (``eval/mrr`` on the validation
window) rather than validation loss; this is a deliberately stronger criterion
than the paper's NLL-based selection.

Why this fixes the "tuned worse than default" problem
-----------------------------------------------------
Earlier 5-trial studies frequently selected configurations that were *worse*
than the strong default configs, because (a) the search budget was tiny, (b) the
validation window (n_truth=100) was far noisier than the reporting window
(n_truth=1000), and (c) the default config was never itself a candidate, so
Optuna returned "best of five noisy samples" even when all five lost to the
default. This runner closes all three gaps:

1. **Default-as-candidate (the decisive fix).** ``main_optuna`` enqueues the
   base config as an explicit, protected trial (``hpo.enqueue_default``). The
   selected best trial is therefore guaranteed to be no worse than the default
   on the validation criterion — tuning can match the default but never lose to
   it. When tuning does not help, the paired ``<method>`` and ``<method>_optuna``
   rows simply coincide, which is the honest scientific outcome.
...
TUNER_DEFAULTS: dict = {
    ...
    "hpo_scope": "network",
    "hpo_reference_r0": "r0_20",  # R0 ~= 2, matching the paper's regime
    ...
    # main failure mode of the earlier 5-trial studies).
    "hpo_enqueue_default": True,
    # Tune on a validation window the same size as the final test window so
    # selection is not noisier than reporting (the earlier 100-vs-1000 mismatch
    # made best-trial selection unreliable).
    "hpo_n_truth": 250,
    "hpo_n_mc": 300,
    "hpo_epochs": 300,
    "hpo_patience": 30,
