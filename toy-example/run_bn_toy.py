...
class Config:
    ...
    n_runs:         int   = 5000    # ground-truth SIR runs per source node
    # BN architecture
    hidden_dim:     int   = 32     # hidden embedding size D
    num_layers:     int   = 6      # number of BN convolutional layers L
    # Optimiser
    lr:             float = 1e-3
...
def main() -> None:
    ...
    print(f"   Unique edges: {G.number_of_edges()}  "
      f"({'directed graph' if cfg.directed else 'undirected graph'})")
